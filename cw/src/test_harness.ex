defmodule TestHarness do
  ...
  defp wait_for_set(procs, _, q, name, _) do
    ...
    s =
      Enum.reduce(procs, MapSet.new(), fn p, s ->
        if :global.whereis_name(p) != :undefined, do: MapSet.put(s, p), else: s
      end)

    (fn d -> wait_for_set(d, MapSet.size(d), q, name, name in d) end).(
      MapSet.difference(procs, s)
    )
  end
  ...
  def get_os_pid(node) do
    ...
    after
      1000 -> get_os_pid(node)
    end
  end

  def deploy_procs(func, config) do
    os_pids =
      for node <- MapSet.new(nodes(config)) do
        cmd =
          "elixir --sname " <>
            hd(String.split(Atom.to_string(node), "@")) <> " --no-halt --detached"
